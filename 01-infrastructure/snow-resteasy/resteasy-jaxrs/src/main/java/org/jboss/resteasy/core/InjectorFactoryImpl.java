...
public class InjectorFactoryImpl implements InjectorFactory {
    ...
    public ValueInjector createParameterExtractor(Parameter parameter, ResteasyProviderFactory providerFactory) {
        switch (parameter.getParamType()) {
            case JSON_DATA: {
                Class<?> classZ = parameter.getType();
                try {
                    if (parameter.getType().getMethod("toArray") != null) {
                        classZ = (Class<?>) ((ParameterizedType) parameter.getGenericType()).getActualTypeArguments()[0];
                    }
                } catch (Exception e) {
                }

                return new AutoInjector(classZ, parameter.getGenericType(), parameter.getParamName(), parameter.getAnnotations());
            }
            case QUERY_PARAM:
            ...
            case MESSAGE_BODY:
                return new DefaultInjector(parameter.getType(), parameter.getGenericType(), parameter.getParamName(), parameter.getAnnotations());
    ...
    public ValueInjector createParameterExtractor(Class injectTargetClass, AccessibleObject injectTarget, Class type, Type genericType, Annotation[] annotations, boolean useDefault, ResteasyProviderFactory providerFactory) {
        ...
        if ((queryParam = findAnnotation(annotations, QueryParam.class)) != null) {
            ...
            //return new DefaultInjector(injectTargetClass,genericType,)
        } else {
            return null;
        }
    }
