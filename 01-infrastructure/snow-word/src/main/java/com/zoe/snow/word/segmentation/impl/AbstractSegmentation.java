...
import com.zoe.snow.word.corpus.Bigram;
import com.zoe.snow.word.corpus.Trigram;
import com.zoe.snow.word.dictionary.Dictionary;
import com.zoe.snow.word.dictionary.DictionaryFactory;
import com.zoe.snow.word.recognition.PersonName;
import com.zoe.snow.word.recognition.Punctuation;
import com.zoe.snow.word.segmentation.*;
import com.zoe.snow.word.tagging.SynonymTagging;
...
public abstract class AbstractSegmentation implements DictionaryBasedSegmentation {
     ...
     * @return 分词结果
     */
    public abstract List<Word> segImpl(String text) throws IOException;
     ...
     * @date 2016/9/7
     * 判断一个词在字典中是否存在，如果存在，放入队列中
     * @param result 队列
     * @param text 文本
     */
    protected void addCombinationWord(List<Word> result, String text) {
        Boolean isExist=getDictionary().contains(text, 0, text.length());
        if(isExist){
