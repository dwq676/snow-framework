package com.zoe.snow.context.request;
 ...
 * @date 2016/1/18
 */
@Component("snow.context.http-request")
public class RequestImpl implements Request, HttpServletRequestAware {
    ...
    public String getIp() {
        ...
        if (requestThreadLocal.get() == null)
            return "";
        String ip = requestThreadLocal.get().getHeader("x-forwarded-for");
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = requestThreadLocal.get().getHeader("Proxy-Client-IP");
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = requestThreadLocal.get().getHeader("WL-Proxy-Client-IP");
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = requestThreadLocal.get().getHeader("HTTP_CLIENT_IP");
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = requestThreadLocal.get().getHeader("HTTP_X_FORWARDED_FOR");
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = requestThreadLocal.get().getRemoteAddr();
        }
        return ip;
    }
