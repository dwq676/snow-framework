...
public abstract class JdbcSupport<T extends PreparedStatement> implements Jdbc {
    ...
    @Override
    public int update(String sql, Object[] args, String... datasource) {
        if (Logger.isDebugEnable())
            Logger.debug("成功执行SQL[{}:{}]更新操作。", sql, Converter.toString(args));

        try {
            T pstmt = newPreparedStatement(Mode.Write, sql, datasource);
            setArgs(pstmt, args);
    ...
    protected abstract T newPreparedStatement(Mode mode, String sql, String... datasource) throws SQLException;

    protected Connection getConnection(Mode mode, String... datasource) throws SQLException {
        Connection connection = connectionManage.get(mode, datasource);
        if (connection == null)
